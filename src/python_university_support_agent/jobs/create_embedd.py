...
async def get_documents_from_file(file_abs_path: Path | str, filename: str) -> list[Document]:
    ...
    if ext == ".pdf":
        ...
        docx = DocxDocument( BytesIO( file_path ))
...
async def create_embedd(ctx, doc_id):
    ...
    doc: DocumentModel = db.query(DocumentModel).filter(DocumentModel.id == doc_id).first()
    ...
    logger.info(f"File {filename} embedding has processed sucessfully")
